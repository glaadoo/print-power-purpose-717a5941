 ...
 */
export function calculateOrderShipping(items: Array<{ productName: string; category?: string | null }>): number {
  if (items.length === 0) return 0;
  
  let maxShipping: number = SHIPPING_TIERS.LOW;
  
  for (const item of items) {
    const itemShipping = getShippingTier(item.productName, item.category);
