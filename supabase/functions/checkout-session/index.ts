...
serve(async (req) => {
  ...
  try {
    ...
    if (wantsCartFlow) {
      ...
      // Only add donation line item if donation amount is specified
      if (donationCents > 0) {
        ...
        form.set(`line_items[${donationIdx}][price_data][product_data][description]`, `Support ${causeName}`);
      }
    ...
    if (wantsDbFlow) {
      ...
      if (donationCents > 0) {
        ...
        form.set("line_items[1][price_data][product_data][description]", `Support ${cause.name}`);
      }
    ...
    if (donationUsd > 0) {
      ...
      form.set("line_items[1][price_data][unit_amount]", String(donationCents));
      form.set("line_items[1][price_data][product_data][name]", "Optional Donation");
      form.set("line_items[1][price_data][product_data][description]", "Support for your selected cause");
