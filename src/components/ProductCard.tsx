...
export default function ProductCard({
  ...
  const [imageSrc, setImageSrc] = useState<string | null>(product.image_url || product.generated_image_url || null);
  const [isGenerating, setIsGenerating] = useState(false);

  useEffect(() => {
    // Try to get or generate image if none exists
    if (!imageSrc && !imageError && !isGenerating) {
  ...
  }, [product, imageSrc, imageError, isGenerating]);
          ...
          {imageSrc && !imageError ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-white/5">
              <div className="text-center p-4">
                {isGenerating ? (
                  <>
                    <div className="w-16 h-16 mx-auto border-4 border-white/20 border-t-white/60 rounded-full animate-spin"></div>
                    <p className="text-white/40 text-xs mt-2">Generating image...</p>
                  </>
                ) : (
                  <>
